...
object ExtensionHandler {
    ...
    private fun handleChapters(params: JsonObject): JsonElement {
        ...
        System.err.println("[ExtensionHandler] Source: ${source::class.qualifiedName}, baseUrl: ${(source as? HttpSource)?.baseUrl ?: "N/A"}")

        val chapters = try {
            source.fetchChapterList(manga).toBlocking().first()
        } catch (e: Throwable) {
            System.err.println("[ExtensionHandler] fetchChapterList EXCEPTION: ${e::class.simpleName}: ${e.message}")
        ...
        if (chapters.isEmpty()) {
            System.err.println("[ExtensionHandler] WARNING: Chapter list is empty! The site may require Cloudflare/WebView bypass.")
        }
